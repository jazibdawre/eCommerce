...
const filterProducts = async (args) => {
  try {
    const { filters } = args;
    ...
    const relatedProds = prods.filter((prod) => prod.confidenceScore > 6);
    ...
    if(filters.price) {
      ...
      const priceProds = result.filter((prod) => prod.price >= priceL && prod.price <= priceH);
    ...
    if(filters.rating) {
      ...
      const rateProds = result.filter((prod) => prod.rating >= rateL && prod.rating <= rateH);
